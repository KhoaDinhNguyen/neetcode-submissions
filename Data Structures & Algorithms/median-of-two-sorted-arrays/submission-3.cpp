...
public:
    double solution(vector<int>& nums1, vector<int>& nums2) {
        ...
        while (true) {
            int i = l + (r - l) / 2;
            int j = half - i;
            // i represent the number of elements we take
            // if we only check i + 1  and i, we could miss i = 0 where we do not take any element at all
            // because nums[0] still exists
